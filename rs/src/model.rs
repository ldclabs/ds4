...
pub fn build_test_model_with_value(fill: f32) -> OwnedModelWeights {
    ...
    for i in 0..n_layer {
        ...
        layers.push(LayerWeights {
            ...
            indexer_attn_q_b:     tf(&mut buffers, alloc_f16_buf(n_lora_q*n_indexer_head*indexer_head_dim, fill_f16), &format!("{}indexer.attn_q_b.weight", p), 1),
            indexer_proj:         tf(&mut buffers, alloc_f16_buf(n_embd*n_indexer_head, fill_f16), &format!("{}indexer.proj.weight", p), 1),
